...
template <class T> std::string Str(const std::vector<T> &x) {
  std::stringstream ss;
  ss << "(";
  if (!x.empty())
    ss << x[0];
...
template <class T> std::string Str(const std::set<T> &x) {
  std::stringstream ss;
  ss << "{";
...
template <class K, class V> std::string Str(const typename std::map<K, V> &x) {
  std::stringstream ss;
  ss << "{";
