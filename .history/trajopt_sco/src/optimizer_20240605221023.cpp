...
void Optimizer::initialize(const DblVec &x) {
  ...
  if (prob_->getVars().size() != x.size())
    PRINT_AND_THROW(
        boost::format(
            "initialization vector has wrong length. expected %i got %i") %
        prob_->getVars().size() % x.size());
  results_.clear();
  results_.x = x;
}

BasicTrustRegionSQP::BasicTrustRegionSQP(const OptProb::Ptr &prob) {
  ctor(prob);
}
void BasicTrustRegionSQP::setProblem(OptProb::Ptr prob) { ctor(prob); }
void BasicTrustRegionSQP::setParameters(
    const BasicTrustRegionSQPParameters &param) {
  param_ = param;
}
const BasicTrustRegionSQPParameters &
BasicTrustRegionSQP::getParameters() const {
  return param_;
}
BasicTrustRegionSQPParameters &BasicTrustRegionSQP::getParameters() {
  return param_;
}
...
void BasicTrustRegionSQP::adjustTrustRegion(double ratio) {
  setTrustRegionSize(param_.trust_box_size * ratio);
}
void BasicTrustRegionSQP::setTrustRegionSize(double trust_box_size) {
  param_.trust_box_size = trust_box_size;
}
...
DblVec BasicTrustRegionSQP::evaluateCosts(const std::vector<Cost::Ptr> &costs,
...
DblVec BasicTrustRegionSQP::evaluateConstraintViols(
    const std::vector<Constraint::Ptr> &cnts, const DblVec &x) const {
...
std::vector<ConvexObjective::Ptr>
BasicTrustRegionSQP::convexifyCosts(const std::vector<Cost::Ptr> &costs,
                                    const DblVec &x, Model *model) const {
...
std::vector<ConvexConstraints::Ptr> BasicTrustRegionSQP::convexifyConstraints(
    const std::vector<Constraint::Ptr> &cnts, const DblVec &x,
    Model *model) const {
...
DblVec BasicTrustRegionSQP::evaluateModelCosts(
    const std::vector<ConvexObjective::Ptr> &costs, const DblVec &x) const {
  DblVec out(costs.size());
  for (size_t i = 0; i < costs.size(); ++i)
    out[i] = costs[i]->value(x);
...
DblVec BasicTrustRegionSQP::evaluateModelCntViols(
    const std::vector<ConvexConstraints::Ptr> &cnts, const DblVec &x) const {
  DblVec out(cnts.size());
  for (size_t i = 0; i < cnts.size(); ++i)
    out[i] = cnts[i]->violation(x);
...
BasicTrustRegionSQP::getCostNames(const std::vector<Cost::Ptr> &costs) const {
...
std::vector<std::string> BasicTrustRegionSQP::getCntNames(
    const std::vector<Constraint::Ptr> &cnts) const {
...
std::vector<std::string>
BasicTrustRegionSQP::getVarNames(const VarVector &vars) const {
...
DblVec BasicTrustRegionSQPMultiThreaded::evaluateCosts(
    const std::vector<Cost::Ptr> &costs, const DblVec &x) const {
  DblVec out(costs.size());
#pragma omp parallel for schedule(dynamic) num_threads(param_.num_threads)     \
    shared(out, costs, x)
  for (int i = 0; i < static_cast<int>(costs.size()); ++i) {
    out[static_cast<std::size_t>(i)] =
        costs[static_cast<std::size_t>(i)]->value(x);
  }

  return out;
}

DblVec BasicTrustRegionSQPMultiThreaded::evaluateConstraintViols(
    const std::vector<Constraint::Ptr> &cnts, const DblVec &x) const {
  DblVec out(cnts.size());
#pragma omp parallel for schedule(dynamic) num_threads(param_.num_threads)     \
    shared(out, cnts, x)
  for (int i = 0; i < static_cast<int>(cnts.size()); ++i) {
    out[static_cast<std::size_t>(i)] =
        cnts[static_cast<std::size_t>(i)]->violation(x);
  }
...
std::vector<ConvexObjective::Ptr>
BasicTrustRegionSQPMultiThreaded::convexifyCosts(
    const std::vector<Cost::Ptr> &costs, const DblVec &x, Model *model) const {
  std::vector<ConvexObjective::Ptr> out(costs.size());
#pragma omp parallel for schedule(dynamic) num_threads(param_.num_threads)     \
    shared(out, costs, x, model)
  for (int i = 0; i < static_cast<int>(costs.size()); ++i) {
    out[static_cast<std::size_t>(i)] =
        costs[static_cast<std::size_t>(i)]->convex(x, model);
  }
...
BasicTrustRegionSQPMultiThreaded::convexifyConstraints(
    const std::vector<Constraint::Ptr> &cnts, const DblVec &x,
    Model *model) const {
  std::vector<ConvexConstraints::Ptr> out(cnts.size());
#pragma omp parallel for schedule(dynamic) num_threads(param_.num_threads)     \
    shared(out, cnts, x, model)
  for (int i = 0; i < static_cast<int>(cnts.size()); ++i) {
    out[static_cast<std::size_t>(i)] =
        cnts[static_cast<std::size_t>(i)]->convex(x, model);
  }

  return out;
}

DblVec BasicTrustRegionSQPMultiThreaded::evaluateModelCosts(
    const std::vector<ConvexObjective::Ptr> &costs, const DblVec &x) const {
  DblVec out(costs.size());
#pragma omp parallel for schedule(dynamic) num_threads(param_.num_threads)     \
    shared(out, costs, x)
...
DblVec BasicTrustRegionSQPMultiThreaded::evaluateModelCntViols(
    const std::vector<ConvexConstraints::Ptr> &cnts, const DblVec &x) const {
  DblVec out(cnts.size());
#pragma omp parallel for schedule(dynamic) num_threads(param_.num_threads)     \
    shared(out, cnts, x)
...
BasicTrustRegionSQPResults::BasicTrustRegionSQPResults(
    std::vector<std::string> var_names, std::vector<std::string> cost_names,
    std::vector<std::string> cnt_names, const BasicTrustRegionSQP &parent)
    : var_names(std::move(var_names)), cost_names(std::move(cost_names)),
      cnt_names(std::move(cnt_names)), parent_(parent) {
...
void BasicTrustRegionSQPResults::update(
    const OptResults &prev_opt_results, const Model &model,
  ...
  // the Model
  new_x = DblVec(model_var_vals.begin(),
                 model_var_vals.begin() +
                     static_cast<long int>(prev_opt_results.x.size()));

  if (trajopt_common::GetLogLevel() >= trajopt_common::LevelDebug) {
    DblVec cnt_costs1 =
        parent_.evaluateModelCosts(cnt_cost_models, model_var_vals);
    DblVec cnt_costs2 = model_cnt_viols;
    for (unsigned i = 0; i < cnt_costs2.size(); ++i)
      cnt_costs2[i] *= merit_error_coeffs[i];
    LOG_DEBUG("SHOULD BE ALMOST THE SAME: %s ?= %s", CSTR(cnt_costs1),
              CSTR(cnt_costs2));
  ...
  model_merit =
      vecSum(model_cost_vals) + vecDot(model_cnt_viols, merit_error_coeffs);
...
void BasicTrustRegionSQPResults::print() const {
  // Print Header
  std::printf("\n| %s |\n", std::string(88, '=').c_str());
  std::printf("| %s %s %s |\n", std::string(36, ' ').c_str(), "ROS Industrial",
              std::string(36, ' ').c_str());
  std::printf("| %s %s %s |\n", std::string(32, ' ').c_str(),
              "TrajOpt Motion Planning", std::string(31, ' ').c_str());
  std::printf("| %s |\n", std::string(88, '=').c_str());
  ...
  for (size_t i = 0; i < old_cost_vals.size(); ++i) {
    ...
    if (fabs(approx_improve) > 1e-8)
      std::printf("| %10s | %10.3e | %10.3e | %10.3e | %10.3e | %10.3e | "
                  ...
                  model_cost_vals[i], approx_improve, exact_improve,
                  exact_improve / approx_improve, cost_names[i].c_str());
    else
      std::printf("| %10s | %10.3e | %10.3e | %10.3e | %10.3e | %10.3e | %10s "
                  "| %-15s \n",
                  "----------", old_cost_vals[i], new_cost_vals[i],
                  model_cost_vals[i], approx_improve, exact_improve,
                  "----------", cost_names[i].c_str());
  ...
  std::printf("| %s |\n", std::string(88, '=').c_str());
  std::printf(
      "| %10s | %10.3e | %10.3e | %10.3e | %10s | %10s | %10s | SUM COSTS\n",
      "----------", vecSum(old_cost_vals), vecSum(new_cost_vals),
  ...
  if (!cnt_names.empty()) {
    ...
    for (size_t i = 0; i < old_cnt_viols.size(); ++i) {
      ...
      if (fabs(approx_improve) > 1e-8)
        std::printf("| %10.3e | %10.3e | %10.3e | %10.3e | %10.3e | %10.3e | "
                    "%10.3e | %-15s \n",
                    ...
                    exact_improve / approx_improve, cnt_names[i].c_str());
      else
        std::printf("| %10.3e | %10.3e | %10.3e | %10.3e | %10.3e | %10.3e | "
                    "%10s | %-15s \n",
                    merit_error_coeffs[i],
  ...
  std::printf("| %s |\n", std::string(88, '=').c_str());
  std::printf("| %10s | %10.3e | %10.3e | %10.3e | %10s | %10s | %10s | SUM "
              "CONSTRAINTS (WITHOUT MERIT) \n",
              "----------", vecSum(old_cnt_viols), vecSum(new_cnt_viols),
              vecSum(model_cnt_viols), "----------", "----------",
              "----------");
  std::printf("| %s |\n", std::string(88, '=').c_str());
  std::printf("| %10s | %10.3e | %10.3e | %10s | %10.3e | %10.3e | %10.3e | "
              "TOTAL = SUM COSTS + SUM CONSTRAINTS (WITH "
              "MERIT)\n",
              "----------", old_merit, new_merit, "----------",
...
void BasicTrustRegionSQPResults::writeSolver(std::FILE *stream,
                                             bool header) const {
  if (header)
    std::fprintf(stream, "%s,%s,%s,%s,%s,%s\n", "DESCRIPTION", "oldexact",
                 "new_exact", "dapprox", "dexact", "ratio");

  std::fprintf(stream, "%s,%10.3e,%10.3e,%10.3e,%10.3e,%10.3e\n", "Solver",
               old_merit, new_merit, approx_merit_improve, exact_merit_improve,
               merit_improve_ratio);
  std::fflush(stream);
}

void BasicTrustRegionSQPResults::writeVars(std::FILE *stream,
                                           bool header) const {
  if (header) {
...
void BasicTrustRegionSQPResults::writeCosts(std::FILE *stream,
                                            bool header) const {
  if (header) {
    std::fprintf(stream, "%s", "COST NAMES");
    for (const auto &name : cost_names)
      std::fprintf(stream, ",%s,%s,%s,%s", name.c_str(), name.c_str(),
    ...
    for (size_t i = 0; i < cost_names.size(); ++i)
      std::fprintf(stream, ",%s,%s,%s,%s", "oldexact", "dapprox", "dexact",
                   "ratio");
  ...
  for (size_t i = 0; i < old_cost_vals.size(); ++i) {
    ...
    double exact_improve = old_cost_vals[i] - new_cost_vals[i];
    if (fabs(approx_improve) > 1e-8) {
      std::fprintf(stream, ",%e,%e,%e,%e", old_cost_vals[i], approx_improve,
                   exact_improve, exact_improve / approx_improve);
    } else {
      std::fprintf(stream, ",%e,%e,%e,%s", old_cost_vals[i], approx_improve,
                   exact_improve, "nan");
    }
  }
  std::fprintf(stream, "\n");
...
void BasicTrustRegionSQPResults::writeConstraints(std::FILE *stream,
                                                  bool header) const {
  if (header) {
  ...
  for (size_t i = 0; i < old_cnt_viols.size(); ++i) {
    ...
    double exact_improve = old_cnt_viols[i] - new_cnt_viols[i];
    if (fabs(approx_improve) > 1e-8) {
      std::fprintf(stream, ",%e,%e,%e,%e",
                   merit_error_coeffs[i] * old_cnt_viols[i],
                   merit_error_coeffs[i] * approx_improve,
                   merit_error_coeffs[i] * exact_improve,
                   exact_improve / approx_improve);
    } else {
      std::fprintf(stream, ",%e,%e,%e,%s",
                   merit_error_coeffs[i] * old_cnt_viols[i],
                   merit_error_coeffs[i] * approx_improve,
                   merit_error_coeffs[i] * exact_improve, "nan");
    }
  }
  std::fprintf(stream, "\n");
  std::fflush(stream);
...
OptStatus BasicTrustRegionSQP::optimize() {
  ...
  std::vector<double> merit_error_coeffs(constraints.size(),
                                         param_.initial_merit_error_coeff);
  ...
  if (param_.log_results ||
      trajopt_common::GetLogLevel() >= trajopt_common::LevelDebug) {
    log_solver_stream =
        std::fopen((param_.log_dir + "/trajopt_solver.log").c_str(), "w");
    log_vars_stream =
        std::fopen((param_.log_dir + "/trajopt_vars.log").c_str(), "w");
    log_costs_stream =
        std::fopen((param_.log_dir + "/trajopt_costs.log").c_str(), "w");
    log_constraints_stream =
        std::fopen((param_.log_dir + "/trajopt_constraints.log").c_str(), "w");
  }
  ...
  for (int merit_increases = 0;
       merit_increases < param_.max_merit_coeff_increases;
    ...
    for (int iter = 1;; ++iter) { /* sqp loop */
      ...
      if (elapsed_time > param_.max_time) {
        LOG_INFO("Elapsed time %f has exceeded max time %f", elapsed_time,
                 param_.max_time);
        retval = OPT_TIME_LIMIT;
        // || 为或， 这句话判断是否满足约束违反度的要求
        if (results_.cnt_viols.empty() ||
            vecMax(results_.cnt_viols) < param_.cnt_tolerance) {
          retval = OPT_CONVERGED;
          if (!results_.cnt_viols.empty())
            LOG_INFO("woo-hoo! constraints are satisfied (to tolerance %.2e)",
                     param_.cnt_tolerance);
        }
        // 这是C++中的标签语法
        goto cleanup;
      }
      ...
      // that 这一段只在第一次迭代时调用，
      if (results_.cost_vals.empty() &&
          results_.cnt_viols.empty()) { // only happens on the first iteration
      ...
      // DblVec new_cnt_viols = evaluateConstraintViols(constraints,
      // results_.x); DblVec new_cost_vals = evaluateCosts(prob_->getCosts(),
      // results_.x); cout << "costs" << endl; for (int i=0; i <
      // new_cnt_viols.size(); ++i) {
      ...
      while (param_.trust_box_size >= param_.min_trust_box_size) {
        ...
        if (status != CVX_SOLVED) {
          ...
          if (qp_solver_failures == (param_.max_qp_solver_failures - 1)) {
            // convex solver failed and this is the last attempt so setting the
            // trust region to the minimum.
        ...
        iteration_results.update(results_, *model_, cost_models, cnt_models,
                                 cnt_cost_models, constraints,
                                 prob_->getCosts(), merit_error_coeffs);
        if (SUPER_DEBUG_MODE) {
        ...
        if (param_.log_results ||
          ...
          if (log_solver_stream != nullptr)
            iteration_results.writeSolver(log_solver_stream,
                                          results_.n_func_evals == 1);

          if (log_vars_stream != nullptr)
            iteration_results.writeVars(log_vars_stream,
                                        results_.n_func_evals == 1);

          if (log_costs_stream != nullptr)
            iteration_results.writeCosts(log_costs_stream,
                                         results_.n_func_evals == 1);

          if (log_constraints_stream != nullptr)
            iteration_results.writeConstraints(log_constraints_stream,
                                               results_.n_func_evals == 1);
        }
        ...
        if (iteration_results.approx_merit_improve <
        ...
        if (iteration_results.approx_merit_improve /
          ...
          goto penaltyadjustment;
                   ...
                   iteration_results.merit_improve_ratio <
                       param_.improve_ratio_threshold) {
          adjustTrustRegion(param_.trust_shrink_ratio);
          LOG_INFO("shrunk trust region. new box size: %.4f",
                   param_.trust_box_size);
        } else {
          ...
          LOG_INFO("expanded trust region. new box size: %.4f",
                   param_.trust_box_size);
          break;
        }
      }
  ...
  penaltyadjustment:
    if (results_.cnt_viols.empty() ||
      ...
      if (param_.inflate_constraints_individually) {
        ...
        for (std::size_t idx = 0; idx < results_.cnt_viols.size(); idx++) {
          if (results_.cnt_viols[idx] > param_.cnt_tolerance) {
            LOG_INFO("Not all constraints are satisfied. Increasing constraint "
                     "penalties for %s",
                     CSTR(cnt_names[idx]));
      ...
      param_.trust_box_size =
          fmax(param_.trust_box_size,
               param_.min_trust_box_size / param_.trust_shrink_ratio * 1.5);
    }
...
cleanup:
  ...
  results_.total_cost = vecSum(results_.costsco_vals);
  LOG_INFO("\n==================\n%s==================", CSTR(results_));
  callCallbacks();
