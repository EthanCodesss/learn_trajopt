...
void exprToEigen(const QuadExpr &expr,
  ...
  assert((ind2.size() == ind1.size()) &&
         (ind1.size() == expr.coeffs.size())); // NOLINT
  ...
  for (size_t i = 0; i < expr.coeffs.size(); ++i) {
    if (expr.coeffs[i] != 0.0) {
      // NOLINTNEXTLINE
      if (ind1[i] == ind2[i]) {
        triplets.emplace_back(static_cast<Eigen::Index>(ind1[i]),
                              static_cast<Eigen::Index>(ind2[i]),
                              expr.coeffs[i]);
      } else {
        Eigen::Index c{0}, r{0};
        if (ind1[i] < ind2[i]) {
...
void exprToEigen(const AffExprVector &expr_vec,
                 Eigen::SparseMatrix<double> &sparse_matrix,
                 Eigen::VectorXd &vector, Eigen::Index n_vars) {
...
void tripletsToEigen(const IntVec &rows_i, const IntVec &cols_j,
                     ...
                     Eigen::SparseMatrix<double> &sparse_matrix) {
  assert((rows_i.size() == cols_j.size()) &&
         (rows_i.size() == values_ij.size())); // NOLINT
