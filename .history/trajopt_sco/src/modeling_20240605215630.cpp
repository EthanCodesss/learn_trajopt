...
void ConvexObjective::addMax(const AffExprVector &ev) {
  Var m = model_->addVar("max", static_cast<double>(-INFINITY),
...
OptProb::OptProb(ModelType convex_solver,
                 const ModelConfig::ConstPtr &convex_solver_config)
...
VarVector OptProb::createVariables(const std::vector<std::string> &names) {
  return createVariables(names,
                         DblVec(names.size(), static_cast<double>(-INFINITY)),
                         DblVec(names.size(), static_cast<double>(INFINITY)));
}
