...
OSQPModel::OSQPModel(const ModelConfig::ConstPtr &config)
    : P_(nullptr, free), A_(nullptr, free) {
  // tuning parameters to be less accurate, but add a polishing step
  if (config != nullptr) {
    config_.settings =
        std::dynamic_pointer_cast<const OSQPModelConfig>(config)->settings;
  }
}

OSQPModel::~OSQPModel() {
  // The osqp_workspace_ is managed by osqp but its members are not so must
  // clean up.
...
void OSQPModel::updateObjective() {
  ...
  P_.reset(csc_matrix(
      osqp_data_.n, osqp_data_.n, static_cast<c_int>(P_csc_data_.size()),
      P_csc_data_.data(), P_row_indices_.data(), P_column_pointers_.data()));
...
void OSQPModel::updateConstraints() {
  ...
  sm.conservativeResize(
      m_int + n_int,
      Eigen::NoChange_t(
          n_int)); // NOLINT(clang-analyzer-core.UndefinedBinaryOperatorResult)
  ...
  for (std::size_t i_cnt = 0; i_cnt < m; ++i_cnt) {
    l_[i_cnt] = (cnt_types_[i_cnt] == INEQ)
                    ? -OSQP_INFINITY
                    : v[static_cast<Eigen::Index>(i_cnt)];
  ...
  for (size_t k = 0; k < n; ++k) {
    new_inner_sizes[k] =
        sm.innerVector(static_cast<Eigen::Index>(k)).nonZeros() + 1;
  }
  ...
  for (std::size_t i_bnd = 0; i_bnd < n; ++i_bnd) {
    ...
    sm.insert(static_cast<Eigen::Index>(i_bnd + m),
              static_cast<Eigen::Index>(i_bnd)) = 1.;
  }

  eigenToCSC(sm, A_row_indices_, A_column_pointers_, A_csc_data_);

  A_.reset(csc_matrix(
      osqp_data_.m, osqp_data_.n, static_cast<c_int>(A_csc_data_.size()),
      A_csc_data_.data(), A_row_indices_.data(), A_column_pointers_.data()));
...
void OSQPModel::createOrUpdateSolver() {
  ...
  if (ret != 0) {
    ...
    if (ret == OSQP_DATA_VALIDATION_ERROR ||
        ret == OSQP_SETTINGS_VALIDATION_ERROR)
      osqp_workspace_ = nullptr;
    throw std::runtime_error("Could not initialize OSQP: error " +
                             std::to_string(ret));
  }
}
...
void OSQPModel::setVarBounds(const VarVector &vars, const DblVec &lower,
                             const DblVec &upper) {
...
CvxOptStatus OSQPModel::optimize() {
  ...
  if (OSQP_COMPARE_DEBUG_MODE) {
    ...
    Eigen::Map<Eigen::Matrix<c_int, Eigen::Dynamic, 1>> P_p_vec(
        osqp_data_.P->p, osqp_data_.P->n + 1);
    Eigen::Map<Eigen::Matrix<c_int, Eigen::Dynamic, 1>> P_i_vec(
        osqp_data_.P->i, osqp_data_.P->nzmax);
    Eigen::Map<Eigen::Matrix<c_float, Eigen::Dynamic, 1>> P_x_vec(
        osqp_data_.P->x, osqp_data_.P->nzmax);
    ...
    Eigen::Map<Eigen::Matrix<c_int, Eigen::Dynamic, 1>> A_p_vec(
        osqp_data_.A->p, osqp_data_.A->n + 1);
    Eigen::Map<Eigen::Matrix<c_int, Eigen::Dynamic, 1>> A_i_vec(
        osqp_data_.A->i, osqp_data_.A->nzmax);
    Eigen::Map<Eigen::Matrix<c_float, Eigen::Dynamic, 1>> A_x_vec(
        osqp_data_.A->x, osqp_data_.A->nzmax);
    ...
    Eigen::Map<Eigen::Matrix<c_float, Eigen::Dynamic, 1>> l_vec(osqp_data_.l,
                                                                osqp_data_.m);
    Eigen::Map<Eigen::Matrix<c_float, Eigen::Dynamic, 1>> u_vec(osqp_data_.u,
                                                                osqp_data_.m);
  ...
  if (retcode == 0) {
    ...
    solution_ = DblVec(osqp_workspace_->solution->x,
                       osqp_workspace_->solution->x + vars_.size());
    auto status = static_cast<int>(osqp_workspace_->info->status_val);

    if (OSQP_COMPARE_DEBUG_MODE) {
      Eigen::IOFormat format(5);
      Eigen::Map<Eigen::VectorXd> solution_vec(
          solution_.data(), static_cast<Eigen::Index>(solution_.size()));
