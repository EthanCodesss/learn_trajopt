...
public:
  ...
  VarVector createVariables(const std::vector<std::string> &names,
                            const DblVec &lb, const DblVec &ub);
  ...
  /**
   * @brief Pushes the input vector within joint limits by some amount in a
   * repeatable fashion
   * @param x Input vector. If it is already within joint limits, it will not be
   * changed
   * @param delta (Default = 1e-3) Amount that it is pushed within the joint
   * limits. Pushing it exactly onto a limit can cause numerical problems
   * @return The input vector x with values modified to be within the joint
   * limits
   */
...
// 定义了一个模版函数，模版函数允许你定义一个函数，该函数可以操作多种类型的数据，而无需为每种数据类型编写和维护单独的函数版本
template <typename VecType>
inline void setVec(DblVec &x, const VarVector &vars, const VecType &vals) {
  Model assert(vars.size() == vals.size());
  for (size_t i = 0; i < vars.size(); ++i) {
    // static_cast是四种强制类型转换运算符之一
