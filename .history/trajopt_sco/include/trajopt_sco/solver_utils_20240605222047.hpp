 ...
 *                            `QuadExpr`. It will be resized to the correct
 * size.
 * @param [in] n_vars the number of variables in expr. It is usually equal
 *                    to `expr.size()`, but it might be larger.
 ...
 *                            diagonal of the sparse matrix, adding `0.` if
 * needed
 */
void exprToEigen(const QuadExpr &expr,
                 Eigen::SparseMatrix<double> &sparse_matrix,
                 Eigen::VectorXd &vector, const Eigen::Index &n_vars,
 ...
 * @param [out] sparse_matrix matrix where to store results. It will be resized
 *                            to the correct size.
 * @param [out] vector vector where to store all the constants of each
 * `AffExpr`, ordered by their appearance order in `expr_vec`. It will be
...
void exprToEigen(const AffExprVector &expr_vec,
                 Eigen::SparseMatrix<double> &sparse_matrix,
                 Eigen::VectorXd &vector, Eigen::Index n_vars = -1);
/**
 ...
 *                               be guessing the right size of sparse_matrix
 * from a sparse triplet representation.
 */
void tripletsToEigen(const IntVec &rows_i, const IntVec &cols_j,
...
void eigenToTriplets(const Eigen::SparseMatrix<double> &sparse_matrix,
                     IntVec &rows_i, IntVec &cols_j, DblVec &values_ij);
...
void eigenToCSC(Eigen::SparseMatrix<double> &sparse_matrix,
                std::vector<T> &row_indices, std::vector<T> &column_pointers,
                DblVec &values) {
