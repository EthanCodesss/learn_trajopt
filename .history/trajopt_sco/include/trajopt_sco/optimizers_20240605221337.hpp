...
struct BasicTrustRegionSQPParameters {
  ...
  double max_iter = 50;
  /** @brief If improvement is less than improve_ratio_threshold, shrink trust
   * region by this ratio */
  double trust_shrink_ratio = 0.1;
  /** @brief If improvement is less than improve_ratio_threshold, expand trust
   * region by this ratio */
  double trust_expand_ratio = 1.5;
  /** @brief After convergence of penalty subproblem, if constraint violation is
   * less than this, we're done */
  ...
  /** @brief Max number of times the QP solver can fail before optimization is
   * aborted */
  ...
  /** @brief Initial coefficient that is used to scale the constraints. The
   * total constaint cost is constaint_value * coeff * merit_coeff */
  double initial_merit_error_coeff = 10;
  /** @brief If true, merit coeffs will only be inflated for the constaints that
   * failed. This can help when there are lots of constraints */
...
public:
  ...
  // Utility functions, exposed to allow overriding for multi threaded
  // implementations
  virtual DblVec evaluateCosts(const std::vector<Cost::Ptr> &costs,
                               const DblVec &x) const;

  virtual DblVec
  evaluateConstraintViols(const std::vector<Constraint::Ptr> &cnts,
                          const DblVec &x) const;

  virtual std::vector<ConvexObjective::Ptr>
  convexifyCosts(const std::vector<Cost::Ptr> &costs, const DblVec &x,
                 Model *model) const;

  virtual std::vector<ConvexConstraints::Ptr>
  convexifyConstraints(const std::vector<Constraint::Ptr> &cnts,
                       const DblVec &x, Model *model) const;

  virtual DblVec
  evaluateModelCosts(const std::vector<ConvexObjective::Ptr> &costs,
                     const DblVec &x) const;

  virtual DblVec
  evaluateModelCntViols(const std::vector<ConvexConstraints::Ptr> &cnts,
                        const DblVec &x) const;

  virtual std::vector<std::string>
  getCostNames(const std::vector<Cost::Ptr> &costs) const;

  virtual std::vector<std::string>
  getCntNames(const std::vector<Constraint::Ptr> &cnts) const;
...
public:
  ...
  // Utility functions, exposed to allow overriding for multi threaded
  // implementations
  DblVec evaluateCosts(const std::vector<Cost::Ptr> &costs,
                       const DblVec &x) const override final;

  DblVec evaluateConstraintViols(const std::vector<Constraint::Ptr> &cnts,
                                 const DblVec &x) const override final;
  ...
  convexifyConstraints(const std::vector<Constraint::Ptr> &cnts,
                       const DblVec &x, Model *model) const override final;

  DblVec evaluateModelCosts(const std::vector<ConvexObjective::Ptr> &costs,
                            const DblVec &x) const override final;

  DblVec evaluateModelCntViols(const std::vector<ConvexConstraints::Ptr> &cnts,
                               const DblVec &x) const override final;
};
 ...
 *
 * Variable with model_ indicates values calcuated using the Convex
 * approximation of the cost or constraint.
 *
...
struct BasicTrustRegionSQPResults {
  ...
  /** @brief The model (Covex Representation) constraint violations using the
   * new_x values */
  ...
  /** @brief The previous iterations (exact) merit = vecSum(old_cost_vals) +
   * merit_error_coeff * vecSum(old_cnt_viols)
   */
  double old_merit{0};
  /** @brief The models merit = vecSum(model_cost_vals) + merit_error_coeff *
   * vecSum(model_cnt_viols) */
  double model_merit{0};
  /** @brief The exact merit = vecSum(new_cost_vals) + merit_error_coeff *
   ...
   */
  void update(const OptResults &prev_opt_results, const Model &model,
